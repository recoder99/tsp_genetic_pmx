    ...
    def eval_gene(self, text, cost):
        ...
        if len(self.mutation_list) > 0:
            ...
            if temp[0] < self.parent2[0] and temp[0] < self.parent1[0]: #if there is a child better than original parent
                #set the child with lowest cost as parent1
                self.parent1 = temp
                ...
                self.mutation_list = self.mutate(child[0], math.floor(len(temp) * 0.30), math.floor(len(temp) * 0.60)) + self.mutate(child[1], math.floor(len(temp) * 0.30), math.floor(len(temp) * 0.60)) + [child[0]] + [child[1]]
                print(f"New Generation: {self.parent1[1]} Cost: {self.parent1[0]}")
